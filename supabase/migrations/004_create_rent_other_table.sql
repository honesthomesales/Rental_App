...
-- Create index on date for efficient filtering (if not exists)
CREATE INDEX IF NOT EXISTS idx_rent_other_date ON RENT_other(date);

-- Create index on type for efficient filtering (if not exists)
...
DROP TRIGGER IF EXISTS trigger_update_rent_other_updated_at ON RENT_other;
