...
type RentPeriodInsert = Database['public']['Tables']['RENT_rent_periods']['Insert'];
type RentPeriodUpdate = Database['public']['Tables']['RENT_rent_periods']['Update'];
...
export class RentPeriodsService {
  ...
  static async generateRentPeriods(
    ...
    rentCadence: string,
    rentDueDay: number = 1
  ): Promise<RentPeriodGenerationResult> {
    try {
      ...
      if (checkError) {
        return {
          ...
          periods_created: 0,
          periods_updated: 0,
          errors: [handleSupabaseError(checkError)]
        };
      }

      // If periods exist, update them instead of creating new ones
      if (existingPeriods && existingPeriods.length > 0) {
        return await this.updateExistingRentPeriods(
          leaseId,
          tenantId,
          propertyId,
          startDate,
      ...
      const periodInserts: RentPeriodInsert[] = periods.map(period => ({
        tenant_id: tenantId,
      ...
      if (insertError) {
        return {
          ...
          periods_created: 0,
          periods_updated: 0,
          errors: [handleSupabaseError(insertError)]
        };
      }
   ...
   */
  private static async updateExistingRentPeriods(
    leaseId: string,
    tenantId: string,
    propertyId: string,
    startDate: string,
    ...
    try {
      ...
      if (getError) {
        return {
          ...
          periods_created: 0,
          periods_updated: 0,
          errors: [handleSupabaseError(getError)]
        };
      }
  ...
  private static calculateRentPeriods(
    ...
    cadence: string,
    dueDay: number
  ): Array<{ dueDate: string }> {
    const periods: Array<{ dueDate: string }> = [];
   ...
   */
  static async getTenantRentPeriods(tenantId: string): Promise<ApiResponse<RentPeriodRow[]>> {
    try {
      const supabase = getSupabaseClient();
   ...
   */
  static async getPropertyRentPeriods(propertyId: string): Promise<ApiResponse<RentPeriodRow[]>> {
    try {
      const supabase = getSupabaseClient();
   ...
   */
  static async getOverdueRentPeriods(tenantId: string): Promise<ApiResponse<RentPeriodRow[]>> {
    try {
      const supabase = getSupabaseClient();
   ...
   */
  static async updateRentPeriodStatus(
    periodId: string,
    status: string,
    amountPaid?: number,
    lateFeeApplied?: number,
    lateFeeWaived?: boolean
  ): Promise<ApiResponse<RentPeriodRow>> {
    try {
      const supabase = getSupabaseClient();
      
      const updateData: RentPeriodUpdate = {
        status,
        updated_at: new Date().toISOString()
      };
   ...
   */
  static async deleteLeaseRentPeriods(leaseId: string): Promise<ApiResponse<boolean>> {
    try {
      const supabase = getSupabaseClient();
   ...
   */
  static async getTenantRentPeriodSummary(tenantId: string): Promise<ApiResponse<{
