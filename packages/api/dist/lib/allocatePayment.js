...
async function allocatePayment(params) {
    ...
    try {
        ...
        if (existingAllocations && existingAllocations.length > 0) {
            ...
            const applied = await Promise.all(existingAllocations.map(async (allocation) => {
        ...
        for (const period of periods) {
            ...
            const isLate = (0, cadence_1.isPeriodLate)(period.period_due_date) && !period.late_fee_waived;
            const lateFeeAmount = isLate ? (0, cadence_1.getLateFeeAmount)(period.rent_cadence) : 0;
        ...
        if (remainingAmount > 0) {
            ...
            if (lastPeriod) {
                ...
                while (remainingAmount > 0 && futurePeriodsCreated < 12) { // Limit to 12 future periods
                    currentDate = (0, cadence_1.addCadenceDays)(currentDate, lastPeriod.rent_cadence);
